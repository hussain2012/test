...
create unique index if not exists products_product_code_unique on public.products ("productCode") where "productCode" is not null;
...
alter table public.site_settings add column if not exists "policyTitle" text;
alter table public.site_settings add column if not exists "policyText" text;
